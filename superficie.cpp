...
//   FEITO- Adição de operacoes: Escala, Rotacao, Translacao
//   FEITO- Uso de Bases BSplines e CatmullRom
//        - Eficiencia de realismo com triangulacao eficiente
//   FEITO- Combinacao de cores e tonalidades
//NÃO TEM COMO NÃO- Supercicies com maior numero de patchs e formas variadas
//   NÃO TEM COMO NÃO- Melhoria de controle de manupilacao (mouse? ao inves de teclado?)
// ------------------------------------------------------------------------------------
// Atender as 7 OBSERVACOES ....
/*
0 mudar core: FEITO
1 criar matriz base catmullrom: FEITO
2
3 rotação y z: FEITO
4 translada y: FEITO
5 FEITO
...
int clipVertex(int x, int y)
{
	...
	for (i=0; i<pControle->n; i++) 
	{
		for(j=0; j<pControle->m; j++)
		{
			...
			float pontoX=(double)pControle->ponto[i][j][0]/local_scale;
			float pontoY=(double)pControle->ponto[i][j][1]/local_scale;
			float pontoZ=(double)pControle->ponto[i][j][2]/local_scale;
			
			printf("ponto:(%f,%f,%f)\n",pontoX,pontoY,pontoZ);
			// distancia do ponto (x, y) a cada vértice do poligono
			d = sqrt(pow((pontoX-x), 2.0) + pow((pontoY-y), 2.0));
...
/*	SUBmenuGirar = glutCreateMenu(processMenuEvents);
	glutAddMenuEntry("Eixo X", RotarX);
	glutAddMenuEntry("EixoY", RotarY);
	glutAddMenuEntry("Eixo Z", RotarZ);
*/	// ----------------------------------------------------
	// OBSERVACAO 6:
